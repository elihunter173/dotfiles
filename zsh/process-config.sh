...
# Creates an alias from each line of stdin, assuming all comments have been
# removed. Each line is expected to follow the form "alias(1) command(2...)".
create_aliases() {
    sed -e 's/#.*//g' |
        grep --invert-match '^$' |
        sed -e 's/ /="/1' -e 's/^/alias /' -e 's/$/"/'
}
echo "Creating aliases file"
echo '# This script was autogenerated' > ~/.aliases.zsh
create_aliases < ~/.aliases >> ~/.aliases.zsh
